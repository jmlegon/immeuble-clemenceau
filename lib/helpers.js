...
// Postgres répond « relation … does not exist » (ou « schema cache » via
// PostgREST) tant que la migration qui crée la table n'a pas été passée. Trois
// écrans posaient la même question avec la même expression régulière recopiée.
export function tableManquante(erreur) {
  return !!erreur && /relation|does not exist|schema cache/i.test(erreur.message || "");
}

// ---------- LOTS ----------

// Les libellés vivaient dans l'écran des lots : le badge du tableau de bord
// affichait donc la valeur brute de la colonne, « résidentiel-vide ». Ils sont
// ici pour que tous les écrans nomment un lot de la même façon.
...
export function labelTypeLot(cle) {
  return TYPES_LOT.find((t) => t.cle === cle)?.label || cle || "—";
}
...
export function labelPeriodicite(cle) {
  return PERIODICITES.find((p) => p.cle === (cle || "mensuelle"))?.label || cle;
}
...
//
// Le contrôle s'arrête au mois du départ du locataire : au-delà, plus rien
// n'est dû. Tant que ces mois n'étaient qu'une ligne d'alerte on pouvait les
// ignorer ; depuis qu'ils portent un bouton « Encaisser », les proposer
// reviendrait à inviter à enregistrer un loyer qui n'existe pas.
export function moisManquants(lot, paiementsDuLot, fenetre) {
  ...
  return fenetre.filter((mo) =>
    (!debutBail || mo >= debutBail) && (!depart || mo <= depart) && !payes.has(mo));
}
